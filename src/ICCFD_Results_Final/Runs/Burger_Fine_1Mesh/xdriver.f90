program conservative_overset
  ...
  real*8 :: rk(4),dx(nmesh),ainf,cfl,foverlap,sweep(6,3)
...
!  call set_type('linear_advection',ainf)
  ...
  do order = 6,6      ! p-order loop
    ...
    if(s.eq.1) then 
      ...
      call exit(1)  ! turning these off for now
    else
...
!        m2start = -0.5 - dx(2)*.5 !! stress test w/ 95% cut
      ...
      !ntime = nint(1*2d0/(ainf*dt)) ! used for burgers
      ntime = nint(1.4*2d0/(ainf*dt)) ! used for burgers
!      ntime = nint(2d0/(ainf*dt)) ! used for lin adv, assuming lenght of domain is 2
      write(*,*) ' '
      write(*,*) '    h, dx = ',h,dx
...
!      if(nmesh.gt.1) call init_mesh(msh(2),[m2start,m2start+0.75d0],dx(2),0,order)
...
!      if(nmesh.gt.1) call computeMoments(msh(2),mom0(:,2),err(2),nincomp2,elemInfo2)
...
!      if(nmesh.gt.1) call computeMoments(msh(2),mom1(:,2),err(2),nincomp2,elemInfo2)
